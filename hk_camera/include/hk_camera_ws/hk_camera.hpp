...
class HKCamera {
    typedef std::function<void(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo)> callback_func_;
...
public:
    ...
    int start_camera(callback_func_ cb);
    // 停止取流
    int stop_camera();

    static void *work_thread(callback_func_ cb, void* handle);

    void* get_handle() const {return this->handle_;}
...
int HKCamera::initial_camera() {
    ...
    RCLCPP_INFO(rclcpp::get_logger("camera"),"get device num :[%d]", stDeviceList.nDeviceNum);
...
int HKCamera::start_camera(callback_func_ cb) {
    // 设置模式属性
    int nRet = MV_OK;
    ...
    //     return nRet;
    // }
    RCLCPP_INFO(rclcpp::get_logger("camera"), "start camera ret:[%d]",nRet);

    std::thread{work_thread,cb,this->handle_}.join();
    return 0;

}

void *HKCamera::work_thread(callback_func_ cb, void* handle) {
