...
def optical_flow_marker_tracking(imgw,imgh,SAVE_VIDEO_FLAG,USE_MINI_LIVE):
    DRAW_MARKERS = True
    if SAVE_VIDEO_FLAG:
        DRAW_MARKERS = False
    ...
    if USE_MINI_LIVE:
        ...
        ret, f0 = cap.read()
        f0gray = cv2.cvtColor(f0, cv2.COLOR_BGR2GRAY)   #转换为灰度图

    #cv2.imshow('frame', cv2.resize(f0gray, (2*f0gray.shape[1], 2*f0gray.shape[0])))
...
if __name__ == "__main__":
    ...
    optical_flow_marker_tracking(imgw,imgh,SAVE_VIDEO_FLAG,USE_MINI_LIVE)