// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from service_interface_ws:srv/BlackMonkey.idl
// generated code does not contain a copyright notice
#include "service_interface_ws/srv/detail/black_monkey__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
service_interface_ws__srv__BlackMonkey_Request__init(service_interface_ws__srv__BlackMonkey_Request * msg)
{
  if (!msg) {
    return false;
  }
  // hit_type
  // injury
  return true;
}

void
service_interface_ws__srv__BlackMonkey_Request__fini(service_interface_ws__srv__BlackMonkey_Request * msg)
{
  if (!msg) {
    return;
  }
  // hit_type
  // injury
}

bool
service_interface_ws__srv__BlackMonkey_Request__are_equal(const service_interface_ws__srv__BlackMonkey_Request * lhs, const service_interface_ws__srv__BlackMonkey_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // hit_type
  if (lhs->hit_type != rhs->hit_type) {
    return false;
  }
  // injury
  if (lhs->injury != rhs->injury) {
    return false;
  }
  return true;
}

bool
service_interface_ws__srv__BlackMonkey_Request__copy(
  const service_interface_ws__srv__BlackMonkey_Request * input,
  service_interface_ws__srv__BlackMonkey_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // hit_type
  output->hit_type = input->hit_type;
  // injury
  output->injury = input->injury;
  return true;
}

service_interface_ws__srv__BlackMonkey_Request *
service_interface_ws__srv__BlackMonkey_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  service_interface_ws__srv__BlackMonkey_Request * msg = (service_interface_ws__srv__BlackMonkey_Request *)allocator.allocate(sizeof(service_interface_ws__srv__BlackMonkey_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(service_interface_ws__srv__BlackMonkey_Request));
  bool success = service_interface_ws__srv__BlackMonkey_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
service_interface_ws__srv__BlackMonkey_Request__destroy(service_interface_ws__srv__BlackMonkey_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    service_interface_ws__srv__BlackMonkey_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
service_interface_ws__srv__BlackMonkey_Request__Sequence__init(service_interface_ws__srv__BlackMonkey_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  service_interface_ws__srv__BlackMonkey_Request * data = NULL;

  if (size) {
    data = (service_interface_ws__srv__BlackMonkey_Request *)allocator.zero_allocate(size, sizeof(service_interface_ws__srv__BlackMonkey_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = service_interface_ws__srv__BlackMonkey_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        service_interface_ws__srv__BlackMonkey_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
service_interface_ws__srv__BlackMonkey_Request__Sequence__fini(service_interface_ws__srv__BlackMonkey_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      service_interface_ws__srv__BlackMonkey_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

service_interface_ws__srv__BlackMonkey_Request__Sequence *
service_interface_ws__srv__BlackMonkey_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  service_interface_ws__srv__BlackMonkey_Request__Sequence * array = (service_interface_ws__srv__BlackMonkey_Request__Sequence *)allocator.allocate(sizeof(service_interface_ws__srv__BlackMonkey_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = service_interface_ws__srv__BlackMonkey_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
service_interface_ws__srv__BlackMonkey_Request__Sequence__destroy(service_interface_ws__srv__BlackMonkey_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    service_interface_ws__srv__BlackMonkey_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
service_interface_ws__srv__BlackMonkey_Request__Sequence__are_equal(const service_interface_ws__srv__BlackMonkey_Request__Sequence * lhs, const service_interface_ws__srv__BlackMonkey_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!service_interface_ws__srv__BlackMonkey_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
service_interface_ws__srv__BlackMonkey_Request__Sequence__copy(
  const service_interface_ws__srv__BlackMonkey_Request__Sequence * input,
  service_interface_ws__srv__BlackMonkey_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(service_interface_ws__srv__BlackMonkey_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    service_interface_ws__srv__BlackMonkey_Request * data =
      (service_interface_ws__srv__BlackMonkey_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!service_interface_ws__srv__BlackMonkey_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          service_interface_ws__srv__BlackMonkey_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!service_interface_ws__srv__BlackMonkey_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `words`
#include "rosidl_runtime_c/string_functions.h"

bool
service_interface_ws__srv__BlackMonkey_Response__init(service_interface_ws__srv__BlackMonkey_Response * msg)
{
  if (!msg) {
    return false;
  }
  // beat
  // real_injury
  // words
  if (!rosidl_runtime_c__String__init(&msg->words)) {
    service_interface_ws__srv__BlackMonkey_Response__fini(msg);
    return false;
  }
  return true;
}

void
service_interface_ws__srv__BlackMonkey_Response__fini(service_interface_ws__srv__BlackMonkey_Response * msg)
{
  if (!msg) {
    return;
  }
  // beat
  // real_injury
  // words
  rosidl_runtime_c__String__fini(&msg->words);
}

bool
service_interface_ws__srv__BlackMonkey_Response__are_equal(const service_interface_ws__srv__BlackMonkey_Response * lhs, const service_interface_ws__srv__BlackMonkey_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // beat
  if (lhs->beat != rhs->beat) {
    return false;
  }
  // real_injury
  if (lhs->real_injury != rhs->real_injury) {
    return false;
  }
  // words
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->words), &(rhs->words)))
  {
    return false;
  }
  return true;
}

bool
service_interface_ws__srv__BlackMonkey_Response__copy(
  const service_interface_ws__srv__BlackMonkey_Response * input,
  service_interface_ws__srv__BlackMonkey_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // beat
  output->beat = input->beat;
  // real_injury
  output->real_injury = input->real_injury;
  // words
  if (!rosidl_runtime_c__String__copy(
      &(input->words), &(output->words)))
  {
    return false;
  }
  return true;
}

service_interface_ws__srv__BlackMonkey_Response *
service_interface_ws__srv__BlackMonkey_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  service_interface_ws__srv__BlackMonkey_Response * msg = (service_interface_ws__srv__BlackMonkey_Response *)allocator.allocate(sizeof(service_interface_ws__srv__BlackMonkey_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(service_interface_ws__srv__BlackMonkey_Response));
  bool success = service_interface_ws__srv__BlackMonkey_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
service_interface_ws__srv__BlackMonkey_Response__destroy(service_interface_ws__srv__BlackMonkey_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    service_interface_ws__srv__BlackMonkey_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
service_interface_ws__srv__BlackMonkey_Response__Sequence__init(service_interface_ws__srv__BlackMonkey_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  service_interface_ws__srv__BlackMonkey_Response * data = NULL;

  if (size) {
    data = (service_interface_ws__srv__BlackMonkey_Response *)allocator.zero_allocate(size, sizeof(service_interface_ws__srv__BlackMonkey_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = service_interface_ws__srv__BlackMonkey_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        service_interface_ws__srv__BlackMonkey_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
service_interface_ws__srv__BlackMonkey_Response__Sequence__fini(service_interface_ws__srv__BlackMonkey_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      service_interface_ws__srv__BlackMonkey_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

service_interface_ws__srv__BlackMonkey_Response__Sequence *
service_interface_ws__srv__BlackMonkey_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  service_interface_ws__srv__BlackMonkey_Response__Sequence * array = (service_interface_ws__srv__BlackMonkey_Response__Sequence *)allocator.allocate(sizeof(service_interface_ws__srv__BlackMonkey_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = service_interface_ws__srv__BlackMonkey_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
service_interface_ws__srv__BlackMonkey_Response__Sequence__destroy(service_interface_ws__srv__BlackMonkey_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    service_interface_ws__srv__BlackMonkey_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
service_interface_ws__srv__BlackMonkey_Response__Sequence__are_equal(const service_interface_ws__srv__BlackMonkey_Response__Sequence * lhs, const service_interface_ws__srv__BlackMonkey_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!service_interface_ws__srv__BlackMonkey_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
service_interface_ws__srv__BlackMonkey_Response__Sequence__copy(
  const service_interface_ws__srv__BlackMonkey_Response__Sequence * input,
  service_interface_ws__srv__BlackMonkey_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(service_interface_ws__srv__BlackMonkey_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    service_interface_ws__srv__BlackMonkey_Response * data =
      (service_interface_ws__srv__BlackMonkey_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!service_interface_ws__srv__BlackMonkey_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          service_interface_ws__srv__BlackMonkey_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!service_interface_ws__srv__BlackMonkey_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
